from newprior.model import DropPath
...
class refineblock4(nn.Module):
    def __init__(self):
        ...
        self.enhance = nn.Sequential( nn.Conv2d(64, 64, 5, stride=1, padding=2),nn.ReLU(inplace=True),
        ...
        self.input_proj64 = InputProj(in_channel=64, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.input_proj1280 = InputProj(in_channel=1280, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(in_channel=embed_dim, out_channel=64, kernel_size=3, stride=1)
    def forward(self, x, save):
        save = F.interpolate(save, scale_factor=4, mode="bicubic")
        save = self.input_proj1280(save)
        x = self.input_proj64(x)
